...
import { X, Upload, FileText, CheckCircle, AlertCircle, Loader2, Download, HelpCircle } from 'lucide-react';
...
const ProductImportModal: React.FC<ProductImportModalProps> = ({ show, onClose, onSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-blue-500 rounded-lg">
                            <Upload className="w-5 h-5 text-white" />
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-gray-800 dark:text-white">Importar Planilha</h3>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Cadastre seus produtos de forma rápida e fácil</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors group">
                ...
                {/* Footer */}
                <div className="flex justify-end gap-3 p-5 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                    <button
                        disabled={importing}
                        onClick={onClose}
                        className="px-5 py-2.5 text-gray-500 hover:text-gray-800 dark:hover:text-white font-bold text-sm transition-colors"
                    >
                    ...
                    {!result && (
                        <button
                            ...
                            className={`px-8 py-2.5 bg-blue-600 text-white rounded-xl flex items-center gap-2 font-black text-sm transition-all shadow-lg shadow-blue-500/20 active:scale-95 ${!file || importing ? 'opacity-30 grayscale cursor-not-allowed shadow-none' : 'hover:bg-blue-700 hover:-translate-y-0.5'
                                }`}
                        >
                            {importing ? (
                                <>
