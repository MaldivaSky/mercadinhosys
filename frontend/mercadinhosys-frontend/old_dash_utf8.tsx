...
// ­ƒöÑ RESTAURADO: Importar modais
import {
  AnomalyDetailsModal,
...
// ­ƒöÑ NOVO: Interface para M├®tricas de RH
interface RHMetrics {
...
interface DashboardData {
  ...
  data: {
    ...
    rh?: RHMetrics; // ­ƒöÑ NOVO: Dados de RH
  };
}
...
const DashboardPage: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'visao-geral' | 'detalhado' | 'avancado' | 'rh'>('visao-geral'); // ­ƒöÑ Inicia em Vis├úo Geral
  const [hoveredKPI, setHoveredKPI] = useState<number | null>(null);
  const [expandedKPI, setExpandedKPI] = useState<number | null>(null);

  // ­ƒöÑ NOVO: Filtro de per├¡odo
  ...
  // ­ƒöÑ NOVO: Modal de KPI com gr├ífico
  ...
  useEffect(() => {
    loadDashboard();
  }, [periodoDias]); // ­ƒöÑ Recarregar quando per├¡odo mudar
  ...
  // ­ƒöÑ NOVO: Aplicar filtro personalizado
  ...
  const loadDashboard = async () => {
    try {
      ...
      // ­ƒöÑ NOVO: Se o usu├írio filtrou datas espec├¡ficas, enviar essas datas
      ...
      if (dataInicio && dataFim) {
        ...
        console.log(`´┐¢ Carregando dashboard com datas espec├¡ficas: ${dataInicio} a ${dataFim}`);
      } else {
        console.log(`­ƒöì Carregando dashboard com per├¡odo de ${periodoDias} dias`);
      }

      const response = await apiClient.get(url);

      console.log('­ƒöì Backend Response:', response.data);
      console.log('­ƒôè Per├¡odo retornado pelo backend:', response.data?.metadata?.period_days);

      // ­ƒöÑ MAPEAR ESTRUTURA DO BACKEND PARA O FORMATO ESPERADO PELO FRONTEND
      const backendData = response.data.data;
      const financials = backendData?.financials || {};

      // ­ƒöÑ CORRE├ç├âO: Usar dados financeiros pr├®-calculados pelo backend (Unica Fonte de Verdade)
      ...
      const mappedData = {
        ...
        data: {
          hoje: {
            ...
            custo_vendas: cogs, // ­ƒöÑ CORRIGIDO: Usar CMV (Custo das Mercadorias Vendidas)
          ...
          mes: {
            ...
            despesas_por_tipo: {},
            custo_produtos_vendidos: cogs, // ­ƒöÑ CORRIGIDO: Usar CMV
            investimentos: valorEstoqueAtivo, // ­ƒöÑ Manter valor do estoque como investimento (Ativo)
            roi_mensal: roiMensal
          },
          rh: backendData?.rh, // ­ƒöÑ NOVO: Dados de RH mapeados
          analise_produtos: {
          ...
          analise_financeira: {
            ...
            indicadores: {
              ...
              margem_seguranca: totalVendas > 0 ? ((lucroLiquido / totalVendas) * 100) : 0,  // ­ƒöÑ CORRIGIDO: Usar lucroLiquido
          ...
          analise_temporal: {
            vendas_por_hora: (backendData?.sales_by_hour || []).map((h: any) => ({
              ...
              lucro: h.lucro || 0,
              margem: h.margem || 0
            })),  // ­ƒöÑ CORRE├ç├âO: Mapear corretamente com valores padr├úo
            produtos_por_hora: (() => {
              // ­ƒöÑ CORRE├ç├âO: Transformar lista em Record<hora, produtos[]>
              const topProducts = backendData?.top_products_by_hour || [];
              if (!Array.isArray(topProducts)) return {};

              // Se for array simples, agrupar por hora (ou retornar vazio se n├úo tiver hora)
              ...
              return grouped;
            })(),  // ­ƒöÑ NOVO: Produtos por hor├írio
            padroes_temporais_clientes: (() => {
              // ­ƒöÑ CORRE├ç├âO: Backend retorna lista simples, frontend espera objeto com perfis_temporais
              const patterns = backendData?.customer_temporal_patterns || [];
              if (!Array.isArray(patterns)) return {};

              // Transformar lista em objeto com estrutura esperada
              return {
                perfis_temporais: {}  // Vazio por enquanto, j├í que backend n├úo retorna perfis
              };
            })(),  // ­ƒöÑ NOVO
            concentracao_horaria: backendData?.hourly_concentration || {},  // ­ƒöÑ NOVO
            matriz_produto_horario: backendData?.product_hour_matrix || { matrix: [], products: [], hours: [] },  // ­ƒöÑ NOVO
            afinidade_cliente_produto: backendData?.customer_product_affinity || [],  // ­ƒöÑ NOVO
            comportamento_clientes_horario: backendData?.hourly_customer_behavior || { comportamento_por_hora: [], total_horas_analisadas: 0 },  // ­ƒöÑ NOVO
      ...
      console.log('­ƒöì Timeseries:', backendData?.timeseries);
      console.log('­ƒöì Timeseries Formatted:', timeseriesFormatted);
      console.log('­ƒöì Sales by Hour:', backendData?.sales_by_hour);
      ...
      console.log('­ƒöì Analise Temporal Mapeada:', mappedData.data.analise_temporal);
      console.log('­ƒöì Correla├º├Áes:', backendData?.correlations);
      console.log('­ƒöì Insights Cient├¡ficos:', mappedData.data.insights_cientificos);
      setData(mappedData);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4 md:p-6">
      ...
      {/* ­ƒöÑ NOVO: FILTROS DE PER├ìODO E VISUALIZA├ç├âO - ACIMA DAS AN├üLISES */}
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            {modoFiltro === 'rapido' && (
              <div className="flex-1">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  ­ƒôà Per├¡odo de An├ílise
                </label>
                <div className="flex gap-2 flex-wrap">
                  {[
                    { dias: 7, label: '7 dias', desc: '├Ültima semana' },
                    { dias: 15, label: '15 dias', desc: '├Ültimas 2 semanas' },
                    { dias: 30, label: '30 dias', desc: '├Ültimo m├¬s' },
                    { dias: 60, label: '60 dias', desc: '├Ültimos 2 meses' },
                    { dias: 90, label: '90 dias', desc: '├Ültimo trimestre' },
                    { dias: 180, label: '6 meses', desc: '├Ültimo semestre' },
                    { dias: 365, label: '1 ano', desc: '├Ültimo ano' }
                  ].map((periodo) => (
                    <button
                      key={periodo.dias}
                ...
                <p className="text-xs text-gray-500 mt-2">
                  ­ƒôè Analisando dados dos ├║ltimos <strong className="text-blue-600">{periodoDias} dias</strong>
                  {loading && <span className="ml-2 text-blue-600 animate-pulse">ÔÇó Carregando...</span>}
                </p>
              </div>
            )}
            ...
            {modoFiltro === 'personalizado' && (
              <div className="flex-1">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  ­ƒôà Selecione o Per├¡odo Personalizado
                </label>
                ...
                {dataInicio && dataFim && (
                  <p className="text-xs text-gray-500 mt-2">
                    ­ƒôè Per├¡odo selecionado: <strong className="text-blue-600">
            ...
            {/* FILTRO DE MODO DE VISUALIZA├ç├âO */}
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                ­ƒæü´©Å Modo de Visualiza├º├úo
              </label>
              <select
              ...
              >
                ...
                <option value="avancado">­ƒö¼ An├ílise Avan├ºada</option>
                <option value="rh">­ƒæÑ An├ílise de RH</option>
              </select>
            </div>
          </div>
        </div>
      ...
      {/* KPIs PRINCIPAIS - COM INDICADOR DE PER├ìODO */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          {
            title: 'Faturamento',
            periodo: `├Ültimos ${periodoDias} dias`,
            ...
            key: `faturamento-${periodoDias}` // ­ƒöÑ Key ├║nica para for├ºar re-render
          },
          {
            title: 'Lucro L├¡quido',
            periodo: `├Ültimos ${periodoDias} dias`,
            ...
            key: `lucro-${periodoDias}` // ­ƒöÑ Key ├║nica para for├ºar re-render
          },
          {
            title: 'Ticket M├®dio',
            periodo: `├Ültimos ${periodoDias} dias`,
            ...
            key: `ticket-${periodoDias}` // ­ƒöÑ Key ├║nica para for├ºar re-render
          },
          {
            title: 'Despesas',
            periodo: `├Ültimos ${periodoDias} dias`,
            ...
            key: `despesas-${periodoDias}` // ­ƒöÑ Key ├║nica para for├ºar re-render
          }
        ].map((kpi, idx) => (
          <div
            key={kpi.key} // ­ƒöÑ Usar key ├║nica baseada no per├¡odo
            onClick={() => setKpiModalAberto(kpi.title)} // ­ƒöÑ Abrir modal ao clicar
          ...
          >
            ...
            <p className="text-xs opacity-70 mb-1">­ƒôà {kpi.periodo}</p>
      ...
      {/* ­ƒöÑ NOVO: RESUMO EXECUTIVO - VIS├âO GERAL */}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl shadow-lg p-4 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold">­ƒôè Vis├úo Geral Ativa</h3>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
              ...
              ­ƒôè Resumo Executivo - ├Ültimos {periodoDias} dias
            </h2>
            ...
            <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800">
                ­ƒÆí <strong>Dica:</strong> Para an├ílises mais detalhadas, selecione <strong>"An├ílise Detalhada"</strong> ou <strong>"An├ílise Avan├ºada"</strong> nos filtros acima.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ­ƒöÑ INDICADOR DE MODO: AN├üLISE DETALHADA */}
      {viewMode === 'detalhado' && (
        <div className="bg-gradient-to-r from-purple-500 to-pink-600 rounded-xl shadow-lg p-4 text-white mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold">­ƒôê An├ílise Detalhada Ativa</h3>
                <p className="text-sm text-purple-100">Curva ABC, Tend├¬ncias e An├ílise Financeira</p>
              </div>
            </div>
            <div className="text-right">
      ...
      {/* ­ƒöÑ INDICADOR DE MODO: AN├üLISE AVAN├çADA */}
      {viewMode === 'avancado' && (
        <div className="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl shadow-lg p-4 text-white mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold">­ƒö¼ An├ílise Avan├ºada Ativa</h3>
                <p className="text-sm text-cyan-100">Insights Cient├¡ficos, Correla├º├Áes e Previs├Áes</p>
              </div>
            </div>
            <div className="text-right">
          ...
          {expandedCards['analise-temporal'] && (
            <div className="p-6 animate-fadeIn">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-4 text-center">
                    <div className="bg-purple-50 p-3 rounded-lg">
                      ...
                      <p className={`text-lg font-bold ${analise_temporal?.tendencia_vendas?.length > 1 && (analise_temporal?.tendencia_vendas[analise_temporal?.tendencia_vendas.length - 1]?.vendas || 0) > (analise_temporal?.tendencia_vendas[analise_temporal?.tendencia_vendas.length - 2]?.vendas || 0) ? 'text-green-600' : 'text-red-600'}`}>
                        {analise_temporal?.tendencia_vendas?.length > 1 && (analise_temporal?.tendencia_vendas[analise_temporal?.tendencia_vendas.length - 1]?.vendas || 0) > (analise_temporal?.tendencia_vendas[analise_temporal?.tendencia_vendas.length - 2]?.vendas || 0) ? '­ƒôê Alta' : '­ƒôë Baixa'}
                      </p>
                    </div>
                ...
                {/* ­ƒöÑ MOVIDO: COMPARA├ç├âO MENSAL - AGORA ABAIXO DO GR├üFICO */}
      ...
      {/* ­ƒöÑ NOVO: SE├ç├âO DE AN├üLISE DE VENDAS POR HOR├üRIO */}
      {viewMode === 'detalhado' && (
        <div className="bg-white rounded-2xl shadow-xl mb-8 overflow-hidden border border-gray-200">
          <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900">ÔÅ░ An├ílise de Vendas por Hor├írio</h2>
                <p className="text-gray-600">Identifique os melhores hor├írios para vendas e otimize sua opera├º├úo</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            {analise_temporal?.vendas_por_hora && analise_temporal.vendas_por_hora.length > 0 ? (
              <div className="space-y-6">
                {/* Gr├ífico de Barras - Vendas por Hora */}
                <div className="bg-gradient-to-br from-gray-50 to-indigo-50 rounded-xl p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">­ƒôè Volume de Vendas por Hor├írio</h3>
                  <div className="h-[400px]">
                    <ResponsiveContainer width="100%" height="100%">
                            ...
                            return (
                              <div className="bg-white dark:bg-slate-800 p-4 shadow-xl rounded-lg border-2 border-indigo-200">
                                <p className="font-bold text-gray-900 dark:text-gray-100 mb-2">
                                  ­ƒòÉ {label}:00 - {label}:59
                                </p>
                                <div className="space-y-1">
                                  <p className="text-sm text-gray-700 dark:text-gray-300">
                                    ­ƒÆ░ Faturamento: <span className="font-bold text-indigo-600">R$ {data?.total?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                  </p>
                                  <p className="text-sm text-gray-700 dark:text-gray-300">
                                    ­ƒøÆ Vendas: <span className="font-bold text-purple-600">{data?.quantidade}</span>
                                  </p>
                                  <p className="text-sm text-gray-700 dark:text-gray-300">
                                    ­ƒÆÁ Ticket M├®dio: <span className="font-bold text-green-600">R$ {(data?.total / data?.quantidade || 0).toFixed(2)}</span>
                                  </p>
                                  <p className="text-sm text-gray-700 dark:text-gray-300">
                                    ­ƒôê Margem: <span className="font-bold text-blue-600">{data?.margem?.toFixed(1)}%</span>
                                  </p>
                                </div>
                              </div>
                            );
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {(() => {
                    ...
                    // ­ƒöÑ DEFENSIVE: Verificar se h├í dados
                    ...
                    // ­ƒöÑ DEFENSIVE: Garantir que melhorHorario e piorHorario t├¬m valores v├ílidos
                    const melhorMargemSafe = melhorHorario?.margem ?? 0;
                    const piorTotalSafe = piorHorario?.total ?? 0;
                    const melhorTotalSafe = melhorHorario?.total ?? 1; // Evitar divis├úo por zero
                      ...
                      <>
                        ...
                        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                              <p className="text-sm text-green-700 font-medium">­ƒÅå Melhor Hor├írio</p>
                              <p className="text-2xl font-bold text-green-900">{melhorHorario?.hora ?? 0}h</p>
                            </div>
                          </div>
                          <div className="space-y-1 text-sm">
                            <p className="text-gray-700">­ƒÆ░ R$ {(melhorHorario?.total ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                            <p className="text-gray-700">­ƒøÆ {melhorHorario?.quantidade ?? 0} vendas</p>
                            <p className="text-gray-700">­ƒôê {melhorMargemSafe.toFixed(1)}% margem</p>
                          </div>
                          <div className="mt-3 pt-3 border-t border-green-200">
                            <p className="text-xs text-green-700">
                              ­ƒÆí <strong>Dica:</strong> Concentre promo├º├Áes e equipe neste hor├írio
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="bg-gradient-to-br from-red-50 to-pink-50 rounded-xl p-6 border border-red-200">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                              <p className="text-sm text-red-700 font-medium">ÔÜá´©Å Hor├írio Cr├¡tico</p>
                              <p className="text-2xl font-bold text-red-900">{piorHorario?.hora ?? 0}h</p>
                            </div>
                          </div>
                          <div className="space-y-1 text-sm">
                            <p className="text-gray-700">­ƒÆ░ R$ {(piorHorario?.total ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                            <p className="text-gray-700">­ƒøÆ {piorHorario?.quantidade ?? 0} vendas</p>
                            <p className="text-gray-700">­ƒôë {((piorTotalSafe / melhorTotalSafe) * 100).toFixed(0)}% do melhor</p>
                          </div>
                          <div className="mt-3 pt-3 border-t border-red-200">
                            <p className="text-xs text-red-700">
                              ­ƒÆí <strong>Dica:</strong> Avalie necessidade de equipe reduzida
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                              <p className="text-sm text-blue-700 font-medium">­ƒÄ» Hor├írios de Pico</p>
                              <p className="text-2xl font-bold text-blue-900">{horariosAcimaDaMedia.length}</p>
                            </div>
                          </div>
                          <div className="space-y-1 text-sm">
                            <p className="text-gray-700">­ƒôè M├®dia: R$ {mediaHoraria.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                            <p className="text-gray-700">ÔÅ░ Hor├írios:</p>
                          ...
                          <div className="mt-3 pt-3 border-t border-blue-200">
                            <p className="text-xs text-blue-700">
                              ­ƒÆí <strong>Dica:</strong> Foque recursos nos hor├írios de pico
                            </p>
                          </div>
                        </div>
                      </>
                ...
                {/* Recomenda├º├Áes Estrat├®gicas */}
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-200">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                    <Lightbulb className="w-5 h-5 text-purple-600" />
                    ­ƒÆí Recomenda├º├Áes Estrat├®gicas
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-white/70 p-4 rounded-lg">
                      <p className="font-semibold text-purple-900 mb-2">­ƒæÑ Gest├úo de Equipe</p>
                      <ul className="text-sm text-gray-700 space-y-1">
                        <li>ÔÇó Escale mais funcion├írios nos hor├írios de pico</li>
                        <li>ÔÇó Reduza equipe nos hor├írios de baixo movimento</li>
                        <li>ÔÇó Planeje intervalos fora dos hor├írios cr├¡ticos</li>
                      </ul>
                    </div>
                    <div className="bg-white/70 p-4 rounded-lg">
                      <p className="font-semibold text-purple-900 mb-2">­ƒÄ» Estrat├®gias de Vendas</p>
                      <ul className="text-sm text-gray-700 space-y-1">
                        <li>ÔÇó Lance promo├º├Áes nos hor├írios de maior movimento</li>
                        <li>ÔÇó Teste ofertas especiais em hor├írios fracos</li>
                        <li>ÔÇó Monitore ticket m├®dio por hor├írio</li>
                      </ul>
                    </div>
                  </div>
                </div>

                {/* ­ƒöÑ NOVO: Produtos Mais Vendidos por Hor├írio */}
                {analise_temporal?.produtos_por_hora && Object.keys(analise_temporal.produtos_por_hora).length > 0 && (
                  <div className="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl p-6 border border-cyan-200">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                      <Package className="w-5 h-5 text-cyan-600" />
                      ­ƒÅå Top Produtos por Hor├írio
                    </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto">
                      {Object.entries(analise_temporal.produtos_por_hora)
                        ...
                        .filter(([hora, produtos]: [string, any]) => {
                          // ­ƒöÑ FILTER: Remover entradas vazias antes do map
                    ...
                    {/* Insights sobre produtos por hor├írio */}
                    <div className="mt-6 bg-white/80 rounded-lg p-4 border border-cyan-100">
                      <h4 className="font-semibold text-cyan-900 mb-3 flex items-center gap-2">
                        ...
                        ­ƒÆí Insights de Produtos por Hor├írio
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700">
                        <div className="flex items-start gap-2">
                          <span className="text-cyan-600">ÔÇó</span>
                          <p>Use esses dados para planejar o estoque e garantir disponibilidade nos hor├írios de pico</p>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="text-cyan-600">ÔÇó</span>
                          <p>Crie combos e promo├º├Áes com os produtos mais vendidos em cada hor├írio</p>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="text-cyan-600">ÔÇó</span>
                          <p>Posicione produtos estrategicamente conforme o hor├írio de maior demanda</p>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="text-cyan-600">ÔÇó</span>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-lg">Dados de vendas por hor├írio n├úo dispon├¡veis</p>
                <p className="text-gray-400 text-sm mt-2">Necess├írio mais dados hist├│ricos para an├ílise</p>
              </div>
            )}
          </div>
        </div>
                ...
                <div key={idx} className={`bg-white/80 backdrop-blur-sm rounded-xl p-6 border ${riscoRuptura ? 'border-red-300 bg-red-50/50' : 'border-purple-100'}`}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-bold ${previsao.classifica├º├úo_abc === 'A' ? 'bg-green-100 text-green-800' :
      ...
      {/* ­ƒöÑ NOVO: AN├üLISE CIENT├ìFICA DE PADR├òES TEMPORAIS */}
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl shadow-xl p-6 mb-8 border border-indigo-200">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {Object.entries(analise_temporal.padroes_temporais_clientes.perfis_temporais || {}).map(([perfil, clientes]: [string, any[]]) => {
              ...
              const perfilConfig = {
                matutino: { icon: '­ƒîà', color: 'from-yellow-50 to-orange-50', border: 'border-yellow-200', text: 'text-yellow-900', badge: 'bg-yellow-100 text-yellow-800' },
                vespertino: { icon: 'ÔÿÇ´©Å', color: 'from-blue-50 to-cyan-50', border: 'border-blue-200', text: 'text-blue-900', badge: 'bg-blue-100 text-blue-800' },
                noturno: { icon: '­ƒîÖ', color: 'from-purple-50 to-indigo-50', border: 'border-purple-200', text: 'text-purple-900', badge: 'bg-purple-100 text-purple-800' }
              }[perfil] || { icon: 'ÔÅ░', color: 'from-gray-50 to-gray-100', border: 'border-gray-200', text: 'text-gray-900', badge: 'bg-gray-100 text-gray-800' };
          ...
          <div className="mt-6 bg-white/80 rounded-xl p-6 border border-indigo-200">
            <h3 className="font-bold text-indigo-900 mb-4 flex items-center gap-2">
              ...
              ­ƒºá Insights Cient├¡ficos sobre Comportamento Temporal
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="flex items-start gap-3 p-3 bg-indigo-50 rounded-lg">
                <span className="text-indigo-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Segmenta├º├úo Temporal:</strong> Clientes t├¬m prefer├¬ncias hor├írias distintas. Use isso para campanhas direcionadas.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-purple-50 rounded-lg">
                <span className="text-purple-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Otimiza├º├úo de Estoque:</strong> Ajuste reposi├º├úo baseado nos perfis temporais dominantes.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
                <span className="text-blue-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Personaliza├º├úo:</strong> Envie ofertas nos hor├írios preferidos de cada segmento para maior convers├úo.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-cyan-50 rounded-lg">
                <span className="text-cyan-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Lifetime Value:</strong> Clientes noturnos tendem a ter maior ticket m├®dio. Priorize reten├º├úo desse segmento.
                </p>
              </div>
            </div>
          </div>
      ...
      {/* ­ƒöÑ NOVO: MATRIZ DE CORRELA├ç├âO PRODUTO ├ù HOR├üRIO */}
      {viewMode === 'avancado' && analise_temporal?.matriz_produto_horario && analise_temporal.matriz_produto_horario.matrix && analise_temporal.matriz_produto_horario.matrix.length > 0 && (
        <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl shadow-xl p-6 mb-8 border border-teal-200">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
            ...
            ­ƒö¼ Matriz de Correla├º├úo: Produto ├ù Hor├írio
          </h2>

          <p className="text-gray-600 mb-6">
            Heatmap mostrando a for├ºa da rela├º├úo entre produtos e hor├írios. Percentuais indicam quanto cada hor├írio contribui para as vendas totais do produto.
          </p>
                    ...
                    {analise_temporal.matriz_produto_horario.hours.map((hora: number) => {
                      ...
                      return (
                        <td
                          ...
                          title={`${produto.produto_nome} ├ás ${hora}h: ${percentual.toFixed(1)}% (${cell?.quantidade || 0} un, R$ ${(cell?.faturamento || 0).toFixed(0)})`}
                        >
          ...
          <div className="mt-6 bg-white rounded-xl p-4 border border-teal-200">
            <h4 className="font-semibold text-teal-900 mb-3">­ƒôè Legenda de Intensidade</h4>
          ...
          <div className="mt-6 bg-white/80 rounded-xl p-6 border border-teal-200">
            <h3 className="font-bold text-teal-900 mb-4 flex items-center gap-2">
              ...
              ­ƒÆí Como Usar Esta An├ílise
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="flex items-start gap-3 p-3 bg-teal-50 rounded-lg">
                <span className="text-teal-600 font-bold">1.</span>
                <p className="text-gray-700">
                  <strong>Gest├úo de Estoque:</strong> Produtos com alta concentra├º├úo em hor├írios espec├¡ficos precisam de reposi├º├úo estrat├®gica.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-cyan-50 rounded-lg">
                <span className="text-cyan-600 font-bold">2.</span>
                <p className="text-gray-700">
                  <strong>Promo├º├Áes Direcionadas:</strong> Lance ofertas nos hor├írios de maior demanda de cada produto.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
                <span className="text-blue-600 font-bold">3.</span>
                <p className="text-gray-700">
                  <strong>Layout da Loja:</strong> Posicione produtos em destaque conforme o hor├írio de pico.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-green-50 rounded-lg">
                <span className="text-green-600 font-bold">4.</span>
                <p className="text-gray-700">
                  <strong>Cross-Selling:</strong> Identifique produtos complementares vendidos no mesmo hor├írio.
                </p>
              </div>
            </div>
          </div>
      ...
      {/* ­ƒöÑ NOVO: AFINIDADE CLIENTE ├ù PRODUTO */}
      {viewMode === 'avancado' && analise_temporal?.afinidade_cliente_produto && analise_temporal.afinidade_cliente_produto.length > 0 && (
        <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-2xl shadow-xl p-6 mb-8 border border-amber-200">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
            ...
            Ô¡É An├ílise de Afinidade: Cliente ├ù Produto
          </h2>

          <p className="text-gray-600 mb-6">
            Identifica padr├Áes de compra recorrentes. Clientes com alta afinidade por produtos espec├¡ficos s├úo alvos ideais para campanhas personalizadas.
          </p>

          {/* Top 20 Afinidades */}
          <div className="bg-white rounded-xl p-6 border border-amber-200">
            <h3 className="font-bold text-amber-900 mb-4">­ƒÄ» Top 20 Afinidades Cliente-Produto</h3>
          ...
          <div className="mt-6 bg-white/80 rounded-xl p-6 border border-amber-200">
            <h3 className="font-bold text-amber-900 mb-4 flex items-center gap-2">
              <Lightbulb className="w-5 h-5" />
              ­ƒÆí Estrat├®gias de Marketing Baseadas em Afinidade
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-lg">
                <span className="text-amber-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Campanhas Personalizadas:</strong> Envie ofertas dos produtos favoritos de cada cliente via WhatsApp.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-yellow-50 rounded-lg">
                <span className="text-yellow-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
              ...
              <div className="flex items-start gap-3 p-3 bg-orange-50 rounded-lg">
                <span className="text-orange-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Recomenda├º├Áes Inteligentes:</strong> Sugira produtos complementares baseado no hist├│rico de afinidade.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-red-50 rounded-lg">
                <span className="text-red-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
      ...
      {/* ­ƒöÑ NOVO: COMPORTAMENTO DE CLIENTES POR HOR├üRIO */}
      {viewMode === 'avancado' && analise_temporal?.comportamento_clientes_horario && analise_temporal.comportamento_clientes_horario.comportamento_por_hora && analise_temporal.comportamento_clientes_horario.comportamento_por_hora.length > 0 && (
        <div className="bg-gradient-to-br from-violet-50 to-purple-50 rounded-2xl shadow-xl p-6 mb-8 border border-violet-200">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
            ...
            ­ƒæÑ Comportamento de Clientes por Hor├írio
          </h2>

          <p className="text-gray-600 mb-6">
            An├ílise detalhada do comportamento de compra dos clientes ao longo do dia. Identifique hor├írios com maior valor por cliente e frequ├¬ncia de compra.
          </p>

          {/* Gr├ífico de Comportamento */}
          <div className="bg-white rounded-xl p-6 border border-violet-200 mb-6">
            <h3 className="font-bold text-violet-900 mb-4">­ƒôè M├®tricas por Hor├írio</h3>
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                        ...
                        <div className="bg-white dark:bg-slate-800 p-4 shadow-xl rounded-lg border-2 border-violet-200">
                          ...
                          <div className="space-y-1 text-sm">
                            <p className="text-gray-700 dark:text-gray-300">
                              ­ƒæÑ Clientes ├Ünicos: <span className="font-bold text-violet-600">{data?.clientes_unicos}</span>
                            </p>
                            <p className="text-gray-700 dark:text-gray-300">
                              ­ƒÆ░ Ticket M├®dio: <span className="font-bold text-green-600">R$ {data?.ticket_medio?.toFixed(2)}</span>
                            </p>
                            <p className="text-gray-700 dark:text-gray-300">
                              ­ƒøÆ Total Vendas: <span className="font-bold text-blue-600">{data?.total_vendas}</span>
                            </p>
                            <p className="text-gray-700 dark:text-gray-300">
                              ­ƒôê Frequ├¬ncia: <span className="font-bold text-purple-600">{data?.frequencia_media?.toFixed(2)} vendas/cliente</span>
                            </p>
                            <p className="text-gray-700 dark:text-gray-300">
                              ­ƒÆÁ Valor/Cliente: <span className="font-bold text-orange-600">R$ {data?.valor_por_cliente?.toFixed(2)}</span>
                            </p>
                          </div>
                        </div>
                      );
          ...
          <div className="bg-white rounded-xl p-6 border border-violet-200 overflow-x-auto">
            <h3 className="font-bold text-violet-900 mb-4">­ƒôï Detalhamento por Hor├írio</h3>
            <table className="w-full text-sm">
              <thead className="bg-violet-50">
                <tr>
                  <th className="p-3 text-left font-bold text-violet-900">Hor├írio</th>
          ...
          <div className="mt-6 bg-white/80 rounded-xl p-6 border border-violet-200">
            <h3 className="font-bold text-violet-900 mb-4 flex items-center gap-2">
              <Lightbulb className="w-5 h-5" />
              ­ƒÆí Insights de Comportamento do Cliente
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="flex items-start gap-3 p-3 bg-violet-50 rounded-lg">
                <span className="text-violet-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Hor├írios VIP:</strong> Hor├írios com maior ticket m├®dio indicam clientes de maior valor. Priorize atendimento premium.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-purple-50 rounded-lg">
                <span className="text-purple-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Frequ├¬ncia de Compra:</strong> Alta frequ├¬ncia indica clientes fi├®is. Crie programas de recompensa para esses hor├írios.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-pink-50 rounded-lg">
                <span className="text-pink-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Valor por Cliente:</strong> M├®trica chave para calcular ROI de campanhas por hor├írio.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-fuchsia-50 rounded-lg">
                <span className="text-fuchsia-600 font-bold">ÔÇó</span>
                <p className="text-gray-700">
                  <strong>Segmenta├º├úo Temporal:</strong> Use esses dados para criar campanhas de marketing direcionadas por hor├írio.
                </p>
              </div>
            </div>
          </div>
      ...
      {/* ­ƒöÑ NOVO: AN├üLISE DE CONCENTRA├ç├âO DE FATURAMENTO */}
      {viewMode === 'avancado' && analise_temporal?.concentracao_horaria && (
        <div className="bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl shadow-xl p-6 mb-8 border border-rose-200">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
            ...
            ­ƒôè An├ílise de Concentra├º├úo: ├ìndice de Gini & Diversifica├º├úo
          </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-white rounded-xl p-6 border border-rose-200">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-rose-900">├ìndice de Gini</h3>
              ...
              <p className="text-sm text-gray-600 mb-2">
                Mede a desigualdade na distribui├º├úo de vendas ao longo do dia
              </p>
              <div className="bg-rose-50 p-3 rounded-lg">
                <p className="text-xs font-semibold text-rose-900 mb-1">Interpreta├º├úo:</p>
                <p className="text-xs text-rose-700">
                  {(analise_temporal.concentracao_horaria.gini_index || 0) > 0.7
                    ? '­ƒö┤ Alta concentra├º├úo - Vendas muito concentradas em poucos hor├írios'
                    : (analise_temporal.concentracao_horaria.gini_index || 0) > 0.4
                      ? '­ƒƒí Concentra├º├úo moderada - Distribui├º├úo razo├ível'
                      : '­ƒƒó Baixa concentra├º├úo - Vendas bem distribu├¡das'}
                </p>
              </div>
            </div>

            {/* Taxa de Concentra├º├úo */}
            <div className="bg-white rounded-xl p-6 border border-orange-200">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-orange-900">Concentra├º├úo Top 3</h3>
              ...
              <p className="text-sm text-gray-600 mb-2">
                Percentual do faturamento nos 3 melhores hor├írios
              </p>
              <div className="bg-orange-50 p-3 rounded-lg">
                <p className="text-xs font-semibold text-orange-900 mb-1">An├ílise:</p>
                <p className="text-xs text-orange-700">
                  {(analise_temporal.concentracao_horaria.concentration_top_3 || 0) > 60
                    ? 'Risco alto - Depend├¬ncia excessiva de poucos hor├írios'
                    : 'Distribui├º├úo saud├ível de vendas'}
                </p>
              </div>
            </div>

            {/* Score de Diversifica├º├úo */}
            <div className="bg-white rounded-xl p-6 border border-green-200">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-green-900">Diversifica├º├úo</h3>
              ...
              <p className="text-sm text-gray-600 mb-2">
                Qu├úo bem distribu├¡das est├úo as vendas ao longo do dia
              </p>
              <div className="bg-green-50 p-3 rounded-lg">
                <p className="text-xs font-semibold text-green-900 mb-1">Status:</p>
                <p className="text-xs text-green-700">
                  {(analise_temporal.concentracao_horaria.diversification || 0) > 60
                    ? 'Ô£à Boa diversifica├º├úo - Menor risco operacional'
                    : 'ÔÜá´©Å Baixa diversifica├º├úo - Considere estrat├®gias para outros hor├írios'}
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Top Hor├írios */}
          <div className="bg-white rounded-xl p-6 border border-rose-200">
            <h3 className="font-bold text-rose-900 mb-4">­ƒÅå Top 5 Hor├írios por Faturamento</h3>
            <div className="space-y-3">
          ...
          <div className="mt-6 bg-white/80 rounded-xl p-6 border border-rose-200">
            <h3 className="font-bold text-rose-900 mb-4 flex items-center gap-2">
              <Lightbulb className="w-5 h-5" />
              ­ƒÆí Recomenda├º├Áes Estrat├®gicas Baseadas em Dados
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="flex items-start gap-3 p-3 bg-rose-50 rounded-lg">
                <span className="text-rose-600 font-bold">1.</span>
                <p className="text-gray-700">
                  <strong>Redu├º├úo de Risco:</strong> Se concentra├º├úo {'>'} 60%, crie promo├º├Áes para hor├írios fracos e diversifique receita.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-pink-50 rounded-lg">
                <span className="text-pink-600 font-bold">2.</span>
                <p className="text-gray-700">
                  <strong>Otimiza├º├úo de Recursos:</strong> Aloque equipe e estoque proporcionalmente ├á concentra├º├úo de vendas.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-purple-50 rounded-lg">
                <span className="text-purple-600 font-bold">3.</span>
                <p className="text-gray-700">
                  <strong>Expans├úo de Hor├írios:</strong> Teste novos produtos/servi├ºos em hor├írios de baixa concentra├º├úo.
                </p>
              </div>
              <div className="flex items-start gap-3 p-3 bg-orange-50 rounded-lg">
                <span className="text-orange-600 font-bold">4.</span>
                <p className="text-gray-700">
                  <strong>Monitoramento Cont├¡nuo:</strong> Acompanhe o ├¡ndice de Gini mensalmente para detectar mudan├ºas de padr├úo.
                </p>
              </div>
            </div>
          </div>
      ...
      {/* ­ƒöÑ NOVO: AN├üLISE TEMPORAL AVAN├çADA - PENSANDO COMO O DONO DO MERCADO */}
      {viewMode === 'avancado' && data?.data && (
        <div className="space-y-8 mb-8">
          {/* SE├ç├âO 1: FATURAMENTO POR PER├ìODO DO DIA */}
          {data.data.period_analysis && Object.keys(data.data.period_analysis).length > 0 && (
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl shadow-xl p-6 border border-blue-200">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
                ...
                ÔÅ░ An├ílise por Per├¡odo do Dia (Manh├ú/Tarde/Noite)
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {Object.entries(data.data.period_analysis).map(([periodo, dados]: [string, any]) => {
                  const periodoNome = periodo === 'manha' ? '­ƒîà Manh├ú (6h-12h)' :
                    periodo === 'tarde' ? 'ÔÿÇ´©Å Tarde (12h-18h)' :
                      '­ƒîÖ Noite (18h-24h)';
          ...
          {/* SE├ç├âO 2: AN├üLISE POR DIA DA SEMANA */}
          {data.data.weekday_analysis && Object.keys(data.data.weekday_analysis).length > 0 && (
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl shadow-xl p-6 border border-green-200">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
                ...
                ­ƒôà An├ílise por Dia da Semana
              </h2>
              <p className="text-gray-600 mb-6">Sexta e s├íbado vendem mais? Qual dia pedir mais estoque?</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {Object.entries(data.data.weekday_analysis).map(([dia, dados]: [string, any]) => {
                  ...
                  const isWeekend = dia === 'Sexta' || dia === 'S├íbado' || dia === 'Domingo';
          ...
          {/* SE├ç├âO 3: RECOMENDA├ç├òES DE ESTOQUE POR HORA */}
          {data.data.product_hourly_recommendations && data.data.product_hourly_recommendations.length > 0 && (
            <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-2xl shadow-xl p-6 border border-orange-200">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
                ...
                ­ƒôª Recomenda├º├Áes de Estoque por Hora
              </h2>
              <p className="text-gray-600 mb-6">Aumentar p├úo amanh├ú? Caprichar em cerveja sexta/s├íbado?</p>
                  ...
                  <div key={idx} className={`rounded-lg p-4 border-l-4 ${rec.prioridade === 'alta' ? 'bg-red-50 border-red-500' :
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{rec.categoria} ÔÇó {rec.periodo}</p>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${rec.prioridade === 'alta' ? 'bg-red-200 text-red-800' :
                        ...
                        {rec.prioridade === 'alta' ? '­ƒö┤ Alta' : rec.prioridade === 'media' ? '­ƒƒí M├®dia' : '­ƒƒó Baixa'}
                      </span>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border border-purple-200">
            ...
            <p className="text-gray-600">
              Vis├úo consolidada de custos de folha, benef├¡cios, assiduidade e horas extras.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100 hover:shadow-lg transition-shadow">
              ...
              <p className="text-xs text-gray-400 mt-2">Sal├írios + Benef├¡cios + Extras</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="p-4 bg-purple-50 rounded-lg border border-purple-100">
                ...
                <p className="text-sm font-bold text-purple-900 mt-2">
                  {rh.resumo_mes?.inicio ? new Date(rh.resumo_mes.inicio).toLocaleDateString('pt-BR') : '-'} ÔÇö {rh.resumo_mes?.fim ? new Date(rh.resumo_mes.fim).toLocaleDateString('pt-BR') : '-'}
                </p>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                Horas Extras por Funcion├írio (M├¬s)
              </h3>
              <div className="overflow-x-auto max-h-[320px] overflow-y-auto">
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 text-gray-600 font-medium sticky top-0">
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 text-gray-600 font-medium sticky top-0">
                    <tr>
                      <th className="px-4 py-3 rounded-l-lg">Funcion├írio</th>
                      <th className="px-4 py-3">Cargo</th>
                      <th className="px-4 py-3">Presen├ºa</th>
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {rh.banco_horas_por_funcionario_mes && rh.banco_horas_por_funcionario_mes.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={4} className="px-4 py-8 text-center text-gray-400">Banco de horas n├úo calculado/registrado</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="overflow-x-auto max-h-[380px] overflow-y-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50 text-gray-600 font-medium sticky top-0">
                  <tr>
                    <th className="px-4 py-3 rounded-l-lg">Funcion├írio</th>
                    <th className="px-4 py-3">Sal├írio</th>
          ...
          {/* Detalhamento Avan├ºado RH */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Gr├ífico de Evolu├º├úo de Turnover */}
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-gray-500" />
                Hist├│rico de Admiss├Áes, Demiss├Áes, Aus├¬ncias, Atrasos e Horas Extras
              </h3>
              <div className="h-[400px]">
                {rh.evolution_turnover && rh.evolution_turnover.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={rh.evolution_turnover}>
                      ...
                      />
                      <Legend />
                      <Bar dataKey="admissoes" name="Admiss├Áes" fill="#10B981" />
                      <Bar dataKey="demissoes" name="Demiss├Áes" fill="#EF4444" />
            ...
            {/* Top Funcion├írios com Horas Extras */}
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
          ...
          {/* Gr├íficos de Composi├º├úo Financeira RH */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Composi├º├úo de Custos */}
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100 lg:col-span-1">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <span className="text-gray-700 font-medium">Sal├írios Base</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                ...
                Resumo Di├írio de Ponto (7 dias)
              </h3>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 text-gray-600 font-medium sticky top-0">
                    <tr>
                      ...
                      <th className="px-4 py-3">Funcion├írio</th>
                    ...
                    {rh.daily_ponto_summary && rh.daily_ponto_summary.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={6} className="px-4 py-8 text-center text-gray-400">
                          Sem registros suficientes para o resumo di├írio
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
              ...
              <div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-100">
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-purple-900 text-sm">Dica de Gest├úo</h4>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 text-gray-600 font-medium">
                    <tr>
                      ...
                      <th className="px-4 py-3 rounded-r-lg text-right">├Ültimo Registro</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                        ...
                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className={`px-2 py-1 text-xs font-bold rounded-full ${func.status === 'Em Trabalho' ? 'bg-green-100 text-green-700' :
                              func.status === 'Almo├ºo' ? 'bg-yellow-100 text-yellow-700' :
          ...
          {/* ├Ültimos Registros de Ponto */}
          <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
              <Clock className="w-5 h-5 text-gray-500" />
              ├Ültimos Registros de Ponto (Listagem)
            </h3>
                          ...
                          <span className={`px-2 py-1 text-xs font-bold rounded-full ${ponto.tipo === 'entrada' ? 'bg-green-100 text-green-700' :
                            ...
                            {ponto.tipo === 'entrada' ? 'Entrada' :
                              ponto.tipo === 'saida' ? 'Sa├¡da' :
                                ponto.tipo === 'saida_almoco' ? 'Sa├¡da Almo├ºo' :
                                  ponto.tipo === 'retorno_almoco' ? 'Retorno Almo├ºo' : ponto.tipo}
                          </span>
                        </td>
                      </tr>
                    ))
      ...
      {/* ­ƒöÑ NOVO: MODAL DE PRODUTO ESTRELA COM PLANO DE A├ç├âO */}
      {modalProdutoEstrela && (
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-yellow-50 to-orange-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">Ô¡É {modalProdutoEstrela.nome}</h2>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
                <h3 className="font-bold text-green-900 mb-4 flex items-center gap-2">
                  ...
                  ­ƒÆÁ An├ílise Financeira Detalhada
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-white/70 p-4 rounded-lg">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                <h3 className="font-bold text-blue-900 mb-4 flex items-center gap-2">
                  ...
                  ­ƒÄ» Plano de A├º├úo Estrat├®gico
                </h3>
                <div className="space-y-3">
                  <div className="bg-white/70 p-4 rounded-lg">
                    <p className="font-semibold text-blue-900 mb-2">1. Gest├úo de Estoque</p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Demanda Di├íria:</strong> {(modalProdutoEstrela.quantidade_vendida / periodoDias).toFixed(1)} unidades/dia
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Estoque Recomendado:</strong> {Math.ceil((modalProdutoEstrela.quantidade_vendida / periodoDias) * 15)} unidades (15 dias)
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Ponto de Reposi├º├úo:</strong> {Math.ceil((modalProdutoEstrela.quantidade_vendida / periodoDias) * 7)} unidades (7 dias)
                    </p>
                  </div>

                  <div className="bg-white/70 p-4 rounded-lg">
                    <p className="font-semibold text-green-900 mb-2">2. Estrat├®gia de Precifica├º├úo</p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Margem Atual:</strong> {modalProdutoEstrela.margem.toFixed(1)}% - {modalProdutoEstrela.margem > 30 ? 'Ô£à Excelente' : modalProdutoEstrela.margem > 20 ? 'Ô£à Boa' : 'ÔÜá´©Å Revisar'}
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Potencial de Aumento:</strong> Teste aumentar 5-10% e monitore vendas
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Combos Sugeridos:</strong> Crie kits com produtos complementares
                    </p>
                  </div>

                  <div className="bg-white/70 p-4 rounded-lg">
                    <p className="font-semibold text-purple-900 mb-2">3. Marketing e Exposi├º├úo</p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Posicionamento:</strong> Coloque em local de destaque (altura dos olhos)
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Promo├º├Áes:</strong> "Leve 3, Pague 2" para aumentar volume
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Comunica├º├úo:</strong> Destaque como "Mais Vendido" ou "Favorito dos Clientes"
                    </p>
                  </div>

                  <div className="bg-white/70 p-4 rounded-lg">
                    <p className="font-semibold text-orange-900 mb-2">4. An├ílise de Fornecedor</p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Negocia├º├úo:</strong> Com alto volume, negocie desconto de 5-10%
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Pagamento:</strong> Solicite prazo maior (30-45 dias)
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Alternativas:</strong> Pesquise 2-3 fornecedores para comparar
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-200">
                <h3 className="font-bold text-purple-900 mb-4 flex items-center gap-2">
                  ...
                  ­ƒôè Proje├º├Áes e Metas
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white/70 p-4 rounded-lg">
                    <p className="text-sm text-gray-600 mb-2">Proje├º├úo Pr├│ximos 30 Dias</p>
                    <p className="text-xl font-bold text-purple-700">
              ...
              <div className="bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl p-6 border border-yellow-200">
                <h3 className="font-bold text-yellow-900 mb-4 flex items-center gap-2">
                  ...
                  ÔÜá´©Å Alertas e Cuidados
                </h3>
                <div className="space-y-2 text-sm text-gray-700">
                  <p>ÔÇó <strong>Ruptura de Estoque:</strong> Nunca deixe faltar! Perda de vendas e clientes.</p>
                  <p>ÔÇó <strong>Validade:</strong> Se perec├¡vel, monitore prazo de validade rigorosamente.</p>
                  <p>ÔÇó <strong>Concorr├¬ncia:</strong> Acompanhe pre├ºos dos concorrentes semanalmente.</p>
                  <p>ÔÇó <strong>Sazonalidade:</strong> Produto pode ter varia├º├úo sazonal - ajuste estoque.</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50">
              ...
              <button
                onClick={() => {
                  alert('Funcionalidade de impress├úo em desenvolvimento');
                }}
      ...
      {/* ­ƒöÑ NOVO: MODAL DE PRODUTO LENTO COM ESTRAT├ëGIAS DE LIQUIDA├ç├âO */}
      {modalProdutoLento && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-red-50 to-pink-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">ÔÜá´©Å {modalProdutoLento.nome}</h2>
                  <p className="text-sm text-gray-600">Produto Lento - Requer A├º├úo Imediata</p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-xl p-6 border border-red-200">
                ...
                <div className="mt-4 bg-red-100 border border-red-300 rounded-lg p-4">
                  <p className="text-sm text-red-900">
                    <strong>ÔÜá´©Å Alerta:</strong> Este produto est├í consumindo capital que poderia ser investido em produtos mais rent├íveis.
                    Tempo estimado para vender estoque atual: <strong>{modalProdutoLento.dias_estoque} dias</strong>
                  </p>
                </div>
              </div>

              {/* Estrat├®gias de Liquida├º├úo */}
              <div className="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-xl p-6 border border-orange-200">
                <h3 className="font-bold text-orange-900 mb-4 flex items-center gap-2">
                  <Target className="w-5 h-5" />
                  ­ƒÄ» Estrat├®gias de Liquida├º├úo (Prioridade Alta)
                </h3>
                <div className="space-y-3">
                  <div className="bg-white/70 p-4 rounded-lg border-l-4 border-red-500">
                    <p className="font-semibold text-red-900 mb-2">1. Promo├º├úo Rel├ómpago (A├º├úo Imediata)</p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Desconto Sugerido:</strong> 30-40% OFF por 7 dias
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Comunica├º├úo:</strong> "Queima de Estoque - ├Ültimas Unidades!"
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Meta:</strong> Vender pelo menos 50% do estoque em 1 semana
                    </p>
                  </div>
                  ...
                  <div className="bg-white/70 p-4 rounded-lg border-l-4 border-orange-500">
                    ...
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Combo 1:</strong> "Leve este produto + Produto Estrela" com 20% OFF
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Combo 2:</strong> "Leve 2, Pague 1,5" (50% OFF na 2┬¬ unidade)
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Posicionamento:</strong> Coloque ao lado de produtos de alta rota├º├úo
                    </p>
                  </div>
                  ...
                  <div className="bg-white/70 p-4 rounded-lg border-l-4 border-yellow-500">
                    ...
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Pontos Dobrados:</strong> Ganhe 2x pontos comprando este produto
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Brinde:</strong> Na compra de 3 unidades, ganhe 1 gr├ítis
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Sorteio:</strong> Cada compra = 1 cupom para sorteio mensal
                    </p>
                  </div>
                  ...
                  <div className="bg-white/70 p-4 rounded-lg border-l-4 border-green-500">
                    ...
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>WhatsApp:</strong> Envie oferta exclusiva para top 20 clientes
                    </p>
                    <p className="text-sm text-gray-700 mb-2">
                      ÔÇó <strong>Mensagem:</strong> "Oferta VIP s├│ para voc├¬: {modalProdutoLento.nome} com 35% OFF"
                    </p>
                    <p className="text-sm text-gray-700">
                      ÔÇó <strong>Urg├¬ncia:</strong> "V├ílido apenas hoje at├® 18h"
                    </p>
                  </div>
                </div>
              </div>

              {/* An├ílise Financeira da Liquida├º├úo */}
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                <h3 className="font-bold text-blue-900 mb-4 flex items-center gap-2">
                  <DollarIcon className="w-5 h-5" />
                  ­ƒÆ░ An├ílise Financeira da Liquida├º├úo
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-white/70 p-4 rounded-lg">
                    <p className="text-sm text-gray-600 mb-2">Cen├írio Atual (Sem A├º├úo)</p>
                  ...
                  <div className="bg-white/70 p-4 rounded-lg">
                    ...
                    <p className="text-xs text-gray-600 mt-1">Recupera├º├úo de 70% do capital</p>
                  </div>
                  <div className="bg-white/70 p-4 rounded-lg">
              ...
              {/* Plano de A├º├úo Passo a Passo */}
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-200">
                <h3 className="font-bold text-purple-900 mb-4 flex items-center gap-2">
                  <ChartBar className="w-5 h-5" />
                  ­ƒôï Plano de A├º├úo - Pr├│ximos 7 Dias
                </h3>
                <div className="space-y-3">
                  <div className="flex items-start gap-3 bg-white/70 p-3 rounded-lg">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">Dia 1-2: Prepara├º├úo</p>
                      <p className="text-sm text-gray-700">Crie material de divulga├º├úo, defina desconto, treine equipe</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3 bg-white/70 p-3 rounded-lg">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">Dia 3-5: Lan├ºamento</p>
                  ...
                  <div className="flex items-start gap-3 bg-white/70 p-3 rounded-lg">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">Dia 6-7: Intensifica├º├úo</p>
                      <p className="text-sm text-gray-700">Se n├úo atingir meta, aumente desconto para 40-50%</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-gray-50 to-slate-50 rounded-xl p-6 border border-gray-300">
                <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                  ...
                  ÔÜû´©Å Decis├úo Final (Se N├úo Vender)
                </h3>
                <div className="space-y-2 text-sm text-gray-700">
                  <p>ÔÇó <strong>Doa├º├úo:</strong> Doe para institui├º├Áes e obtenha benef├¡cio fiscal</p>
                  <p>ÔÇó <strong>Troca com Fornecedor:</strong> Negocie troca por produtos de maior giro</p>
                  <p>ÔÇó <strong>Venda em Lote:</strong> Venda todo estoque para outro comerciante com desconto maior</p>
                  <p>ÔÇó <strong>├Ültima Op├º├úo:</strong> Descarte respons├ível e aprenda com o erro</p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
                <h3 className="font-bold text-green-900 mb-4 flex items-center gap-2">
                  <Lightbulb className="w-5 h-5" />
                  ­ƒÆí Li├º├Áes para Evitar no Futuro
                </h3>
                <div className="space-y-2 text-sm text-gray-700">
                  <p>ÔÇó <strong>Teste Pequeno:</strong> Ao comprar produto novo, comece com quantidade m├¡nima</p>
                  <p>ÔÇó <strong>An├ílise de Demanda:</strong> Pesquise se clientes realmente querem o produto</p>
                  <p>ÔÇó <strong>Monitoramento:</strong> Acompanhe vendas semanalmente, n├úo mensalmente</p>
                  <p>ÔÇó <strong>Fornecedor Flex├¡vel:</strong> Prefira fornecedores que aceitem devolu├º├úo</p>
                </div>
              </div>
            </div>
      ...
      {/* ­ƒöÑ NOVO: MODAL DE HIST├ôRICO DE KPI - SIMPLIFICADO */}
      {kpiModalAberto && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-slate-900">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  ­ƒôè Hist├│rico: {kpiModalAberto}
                </h2>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  An├ílise temporal detalhada
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-6">
              ...
              <div className="bg-gradient-to-br from-gray-50 to-blue-50 dark:from-slate-800 dark:to-slate-900 rounded-xl p-6 shadow-inner">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
                  {visualizacaoModal === 'dias' ? '­ƒôê Evolu├º├úo Di├íria (30 dias)' : '­ƒôè Evolu├º├úo Mensal (12 meses)'}
                </h3>
                ...
                {analise_temporal?.tendencia_vendas && analise_temporal.tendencia_vendas.length > 0 ? (
                      ...
                      <LineChart
                        data={
                          visualizacaoModal === 'dias'
                            ? analise_temporal.tendencia_vendas.slice(-30) // ├Ültimos 30 dias
                            : (() => {
                              // Agrupar por m├¬s para visualiza├º├úo mensal
                      ...
                      >
                        ...
                        <XAxis
                          ...
                          tickFormatter={(value) => {
                            ...
                            return value; // J├í formatado para meses
                          }}
                        />
                        <YAxis
                          tick={{ fontSize: 12, fill: '#6B7280' }}
                  ...
                  <div className="h-[400px] flex items-center justify-center bg-white dark:bg-slate-900 rounded-lg">
                    <div className="text-center">
                      ...
                      <p className="text-gray-500 dark:text-gray-400">Dados insuficientes para gerar gr├ífico</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
                  <p className="text-sm text-blue-700 dark:text-blue-400 mb-1 font-medium">­ƒÆ░ Valor Total</p>
                ...
                <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
                  <p className="text-sm text-green-700 dark:text-green-400 mb-1 font-medium">­ƒôê Crescimento</p>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl p-4 border border-purple-200 dark:border-purple-800">
                  <p className="text-sm text-purple-700 dark:text-purple-400 mb-1 font-medium">­ƒôà Per├¡odo</p>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-xl p-4 border border-orange-200 dark:border-orange-800">
                  <p className="text-sm text-orange-700 dark:text-orange-400 mb-1 font-medium">­ƒôè M├®dia Di├íria</p>
